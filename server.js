...
app.post('/generate-study', async (req, res) => {
  ...
  try {
    ...
    const prompt = `
أنت مستشار ائتماني في بنك.  
اكتب "دراسة سوق تفصيلية" للنشاط التالي:
"${standardName}"

المطلوب:

1) المخرج يكون HTML فقط (بدون أى نص خارج الوسوم) باستخدام:
   - عناوين: <h3>
   - قوائم: <ul><li>
   - جداول: <table><thead><tbody><tr><th><td>

2) ركّز أن الدراسة تناسب استخدام "ائتماني" للبنك:
   - تحليل السوق وحجم الطلب.
   - الموردين / العملاء المستهدفين.
   - دورة التشغيل (أيام شراء – تصنيع/خدمة – تخزين – تحصيل).
   - نسب ربحية تقديرية.
   - المخاطر الأساسية وكيفية الحد منها.
   - توصية ائتمانية في النهاية.

الهيكل المقترح:

<h3>1️⃣ نظرة عامة على النشاط</h3>
(فقرة طويلة من 8–12 سطر تقريباً)

<h3>2️⃣ المنتجات أو الخدمات الرئيسية</h3>
<ul>
<li>…</li>
</ul>
...
<table>
<thead>
<tr><th>المرحلة</th><th>الوصف</th><th>عدد الأيام</th></tr>
</thead>
<tbody>
...
<ul>
<li>تكلفة البضاعة إلى المبيعات (% تقريبية)</li>
<li>مصاريف تشغيلية (% تقريبية)</li>
<li>هامش الربح التشغيلي (% تقريبية)</li>
</ul>

<h3>6️⃣ تحليل SWOT</h3>
<ul>
<li><b>نقاط القوة:</b> …</li>
<li><b>نقاط الضعف:</b> …</li>
<li><b>الفرص:</b> …</li>
<li><b>التهديدات:</b> …</li>
</ul>

<h3>7️⃣ التوصية الائتمانية</h3>
(فقرة تلخص رأي المستشار الائتماني في منح التسهيلات، مع مراعاة الضمانات وحجم التدفقات النقدية)

استخدم لغة عربية رسمية وواضحة، وابتعد عن التكرار قدر الإمكان.
    `;
